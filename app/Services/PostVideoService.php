<?php

namespace App\Services;

use App\Models\Post;
use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Str;
use Illuminate\Validation\ValidationException;
use Symfony\Component\Process\Process;

class PostVideoService
{
    public const MAX_DURATION_SECONDS = 15;

    public function store(Post $post, UploadedFile $file, float $clientDurationSeconds): array
    {
        $serverDurationSeconds = $this->detectDurationSeconds($file);
        $durationSeconds = $serverDurationSeconds ?? $clientDurationSeconds;

        if ($durationSeconds > self::MAX_DURATION_SECONDS) {
            throw ValidationException::withMessages([
                'video' => ['Il video non puo superare 15 secondi.'],
            ]);
        }

        $disk = config('services.spoton_video.disk', 'public');
        $directory = trim((string) config('services.spoton_video.directory', 'post-videos'), '/');
        $extension = $file->guessExtension() ?: $file->getClientOriginalExtension() ?: 'mp4';
        $filename = $post->id.'-'.Str::uuid().'.'.$extension;
        $path = $file->storeAs($directory, $filename, $disk);

        return [
            'video_disk' => $disk,
            'video_path' => $path,
            'video_url' => Storage::disk($disk)->url($path),
            'video_mime' => $file->getMimeType(),
            'video_size_bytes' => $file->getSize(),
            'video_duration_seconds' => (int) ceil($durationSeconds),
        ];
    }

    public function deleteForPost(Post $post): void
    {
        if (! $post->video_disk || ! $post->video_path) {
            return;
        }

        Storage::disk($post->video_disk)->delete($post->video_path);
    }

    public function emptyPayload(): array
    {
        return [
            'video_disk' => null,
            'video_path' => null,
            'video_url' => null,
            'video_mime' => null,
            'video_size_bytes' => null,
            'video_duration_seconds' => null,
        ];
    }

    private function detectDurationSeconds(UploadedFile $file): ?float
    {
        $which = new Process(['sh', '-lc', 'command -v ffprobe']);
        $which->setTimeout(5);
        $which->run();

        if (! $which->isSuccessful()) {
            return null;
        }

        $ffprobe = trim($which->getOutput());
        if ($ffprobe === '') {
            return null;
        }

        $process = new Process([
            $ffprobe,
            '-v',
            'error',
            '-show_entries',
            'format=duration',
            '-of',
            'default=noprint_wrappers=1:nokey=1',
            $file->getRealPath(),
        ]);
        $process->setTimeout(10);
        $process->run();

        if (! $process->isSuccessful()) {
            return null;
        }

        $duration = (float) trim($process->getOutput());

        return $duration > 0 ? $duration : null;
    }
}
